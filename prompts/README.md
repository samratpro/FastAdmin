...
When vibe-coding FastAdmin features with an AI assistant, remember these principles:

1. **Mention the Decoupled Architecture**: Always remind the AI that `api/` is a FastAPI backend with its own custom lightweight ORM and that `admin/` is a separate Next.js app on port `7000`.
2. **Auto-Discovery Hooks**: Remind the AI that FastAdmin does *not* use Prisma or manual router registrations. Simply decorating a model class with `@registerAdmin` and placing files inside `api/src/apps/<appName>/` is enough.
