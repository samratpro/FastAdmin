# AI Prompt: Generate Custom FastAdmin Model and App
...
```text
Act as a premium Node.js and TypeScript developer specializing in the FastAdmin framework.
...
Please generate the complete codebase files matching the FastAdmin framework's exact API structures:
...
- File 2: `api/src/apps/[APP_NAME_LOWERCASE]/routes.ts`
  - Implement a default-exported FastAPI routes plugin:
    ```ts
    import { FastAPIInstance } from 'FastAPI';
    import { [MODEL_NAME_CAMELCASE] } from './models';
    
    export default async function [APP_NAME_LOWERCASE]Routes(FastAPI: FastAPIInstance) {
      // Endpoint 1: List all
      FastAPI.get('/api/public/[APP_NAME_LOWERCASE]', async (request, reply) => {
        const items = await [MODEL_NAME_CAMELCASE].objects.all<[MODEL_NAME_CAMELCASE]>().all();
        return { data: items };
      });
      
      // Endpoint 2: Get single by identifier
      FastAPI.get('/api/public/[APP_NAME_LOWERCASE]/:id', async (request, reply) => {
