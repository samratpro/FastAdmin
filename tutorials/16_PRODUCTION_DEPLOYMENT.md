...
Deploying FastAdmin on a Linux VPS: PostgreSQL + API + Admin panel inside Docker, with Nginx as a reverse proxy and SSL.
...
Browser
  ...
  └── https://api.yourdomain.com    →  Nginx  →  localhost:8000  (FastAPI API)
                                                        │
...
# 1. Clone
cd /www/wwwroot
git clone https://github.com/<your-org>/FastAdmin.git
cd FastAdmin
...
`docker-compose.yml` builds `DATABASE_URL` (as `postgresql+asyncpg://...`), `CORS_ORIGIN`, and `NEXT_PUBLIC_API_URL` from these values automatically — you never set those manually.
...
-----------------------------------------------------------------------
FastAdmin-postgres-1    docker-entrypoint.sh …           Up       5432/tcp
FastAdmin-api-1         sh -c alembic upgrade head …     Up       0.0.0.0:8000->8000/tcp
FastAdmin-admin-1       next start -p 3000               Up       0.0.0.0:7000->3000/tcp
```
...
Run this once after the first deploy to create a superuser:
...
The user persists in PostgreSQL across restarts. Change the password immediately after first login.
...
Migrations run before the server starts on every `docker-compose up`. No manual step needed.
...
- **Dynamic Form Presentation**: The FastAdmin panel detects password inputs, initializes them as empty, and displays the premium security placeholder: `"Leave blank to keep current"`.
- **Intelligent Updates**:
...
### Boolean columns show `-` in admin (is_active, is_staff, is_superuser)

**Cause:** A column type mismatch or migration not applied.

**Fix:** Run `alembic upgrade head` inside the container and rebuild the API.
